...
The binary plugin ID is `dev.teyru`, implementation class `dev.teyru.gradle.TeyruPlugin`, and extension name `teyru`. Contract schema is v1; the local development implementation version is `0.1.0-SNAPSHOT`. This freezes local consumer identity but does not claim ownership or availability on Plugin Portal: publication remains `NOT_AUTHORIZED`. `spec/gradle/plugin-contract-v1.json` is the machine contract.
...
teyru {
    ...
    compilerExecutable.set(layout.projectDirectory.file("tools/teyru"))
...
All values are lazy Gradle `Property`/`ListProperty`/`RegularFileProperty`; conventions use provider APIs and are finalized on read at task execution. P09 accepts target release 21 or 25 and rejects a target above the selected launcher/toolchain. Options are normalized ordered inputs; duplicate/conflicting output, release, project-model or diagnostics flags are forbidden because the plugin owns them. No environment `CLASSPATH`, global Teyru config or working-directory discovery changes task identity.
...
Applying `dev.teyru` applies or waits lazily for `java`; it creates `src/main/teyru` and `src/test/teyru` logical source directories without creating them on disk. For source set `main`:

```text
exportTeyruWorkspaceModel ─┐
                            ├→ generateTeyruJava → compileJava → classes → jar/run
src/main/teyru ────────────┤
src/main/java (analysis) ────┘
```

For `test`, `exportTestTeyruWorkspaceModel → generateTestTeyruJava → compileTestJava → test`; test generation also consumes main outputs/model through normal source-set providers. `compileJava` receives `generateTeyruJava.outputDirectory` through `SourceDirectorySet.srcDir(TaskProvider)`, which establishes the dependency without `dependsOn` string wiring. Likewise for test.

Generation consumes Java source files for declaration analysis but never `compileJava` output; therefore there is no `compileJava ↔ generateTeyruJava` cycle. The finite P09 mixed-source strategy is P06 header/source analysis plus one joint javac compilation: generated Teyru Java and consumer Java are both compileJava sources. A cycle assertion walks the realized task graph in TestKit.

Workspace export uses accepted P08 schema/codecs only; the plugin maps Gradle providers into that model and does not fork a second DTO. Outputs are `build/teyru/workspace/main.json` and `test.json`. Source maps are under `build/teyru/source-maps/<sourceSet>/`; generated Java is exactly `build/generated/sources/teyru/<sourceSet>/`. Nothing writes `src/`.
...
`GenerateTeyruJava` is cacheable only after P37 proves relocation; at P09 it must at least be incremental/up-to-date correct. Annotated properties:

- `@InputFiles @PathSensitive(RELATIVE)`: sorted Teyru sources; sorted Java analysis sources; accepted P08 model inputs; compiler distribution files; referenced configuration files.
...
The task stages all generated Java/maps/manifest, verifies hashes and P06 path rules, then atomically publishes. Its ownership manifest lists exact relative path, type, input source fingerprint and output SHA-256. On removed/renamed `.teyru`, only previously manifest-owned stale Java/map outputs are deleted; foreign files, unlisted files, symlinks and modified owned files are retained and cause an actionable collision/ownership failure. A failed compiler run leaves the last accepted output intact and the Gradle task fails.
...
1. `java-calls-teyru`: `src/main/teyru/p/Greeting.teyru`, Java `Main` calls it, `application` run prints expected output; jar contains both classes.
2. `teyru-calls-java`: Java helper used by a Teyru method; `classes` and external `java` process succeed, proving same-module mixed analysis without a cycle.
3. `test-source-set`: Java or Teyru test consumes main Teyru class; `test` executes a real assertion and both generate tasks have correct graph edges.
4. `failure-location`: syntax/type error fails build and log/JSON contains normalized `.teyru` path/range, not only generated Java.
...
Run clean and non-clean sequences. First successful build must not rely on a prior repository build or developer IDE classpath. Test the local plugin version through `includeBuild`; never resolve `dev.teyru` from public Plugin Portal.
...
| P09-02 | Main/test Teyru roots and named generate tasks exist without creating source dirs; test source-set fixture runs. |
...
| P09-06 | Main/test accepted P08 JSON and source maps below `build/teyru`; no `src` mutation or absolute-path leakage. |
| P09-07 | Fresh includeBuild consumer resolves locally while offline; no Plugin Portal publication assumption. |
| P09-08 | TestKit runs build/test/jar/application and external Java process for Java↔Teyru fixtures. |
...
| P09-10 | Syntax and type negatives fail task/build with exact `.teyru` source location and no new partial output. |
...
- `dev.teyru` is frozen for local use only. Public namespace verification and publication credentials remain a later authorization gate, not a reason to stop local implementation.
