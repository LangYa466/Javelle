...
- `GradleWorkspaceInputAdapter` is the deliberately thin Gradle boundary. It captures real Java `main`/`test` source sets, conventional present/missing Teyru roots, generated roots, module identities and the running toolchain without registering tasks or implementing the future P09 compilation plugin.
...
- `./gradlew --no-daemon --dependency-verification=strict :gradle-plugin:spotlessApply :gradle-plugin:test --rerun-tasks` — exit 0; 3 tests, 0 failed/skipped. XML: `gradle-plugin/build/test-results/test/TEST-dev.teyru.gradle.workspace.P08WorkspaceIntegrationTest.xml`.
