...
- P09-08: the fixture runs real Gradle `test` with a Java assertion crossing Java/Javelle/Java, application `run`, jar, and the built classes in an external native JDK 21 process.
...
- P09-10/11: syntax failure retains accepted bytes and exposes original `A.javelle` plus `JV-SYN`; modified-owned, symlink and foreign collision attacks fail closed. Stale generated Java/class/map removal is asserted. Source-map publication now validates ownership and uses a sibling stage, atomic swap and backup rollback.

## Implemented requirements

- P09-01/02/03: binary plugin `org.javelle`, typed lazy `javelle` extension, real main/test `SourceDirectorySet`, provider-backed generate tasks before Java compilation.
- P09-05: `GenerateJavelleJava` uses Gradle `processIsolation()` Worker API and starts only the explicitly configured packaged `javelle` executable. Missing/non-executable input fails before publication.
- P09-06: export tasks serialize the accepted P08 portable model to `build/javelle/workspace/{main,test}.json`; generated Java is under `build/generated/sources/javelle/<sourceSet>`.
- P09-08/09: external TestKit consumer compiles main and test Javelle sources, Java calls generated Javelle, application `run` prints `OK`, jar is built, second identical build is `UP_TO_DATE` with configuration-cache reuse, source mutation reruns, deleted owned output disappears, and an injected foreign file survives.
- P09-04/08: the execution facade accepts declared Java sources and forwards them to the P06 joint javac request. The offline consumer proves `Java Main -> Javelle User -> Java Helper` with the JVM output `OK`.
- P09-06/11: source maps are hash-owned below `build/javelle/source-maps/<sourceSet>`; zero-source regeneration validates prior CLI ownership, removes owned Java/maps, and preserves an unlisted foreign file.
- P09-07/12: the final TestKit consumer resolves `org.javelle` using `pluginManagement.includeBuild(<local checkout>)` from a temporary external project.

## Verification

- `./gradlew --no-daemon :gradle-plugin:spotlessApply :gradle-plugin:test --tests org.javelle.gradle.P09GradlePluginTest --rerun-tasks` — exit 0, 2/2 P09 external cases using offline local `includeBuild`. Evidence `.agent/logs/P09-W02/w03-final-r2.log`.
