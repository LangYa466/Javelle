...
|---|---|---|
| P10-01 | PASS | `javelle-lsp --stdio` installs and launches as a standalone external process (`:language-server:installDist`); `language-server` depends only on `language-protocol`, `language-tooling`, `compiler-driver`/`compiler-core`/`workspace-model` — no IntelliJ jar/class present. |
...
| P10-04 | PASS | `capabilities()` in `JavelleLanguageServerMain` advertises exactly `positionEncoding=utf-16`, incremental sync with `didSave.includeText=false`, `hoverProvider=true`, `completionProvider{resolveProvider=false}` — nothing else. `compatibility/lsp-methods.json` independently verified: 95 unique methods, exactly 12 `VERIFIED`, exactly 6 `advertised=true`, and zero methods where `advertised=true` but `supportStatus!=VERIFIED` (checked via direct JSON parse, not by trusting the file's own claims). |
| P10-05 | PASS | `compilerBackedSyntaxDiagnosticUsesRawUtf16` and the server-level diagnostic fixture assert the exact UTF-16 range for CRLF/emoji-adjacent content and a subsequent empty-array clear at a strictly newer version, against the original `.javelle` URI. |
...
| P10-08 | **PASS (previously FAIL in R2/R3 — now repaired)** | `DocumentWorkspace` exposes an injectable `analysisCheckpoint` (production default is a no-op; `testAnalysisDelayFromEnvironment()` reads `JAVELLE_LSP_TEST_ANALYSIS_MARKER`/`JAVELLE_LSP_TEST_ANALYSIS_CHECKPOINT` env vars to block a specific numbered checkpoint call until the worker thread is interrupted). `analyze()` calls this checkpoint and `cancelled()` (which throws `CancellationException` if `Thread.currentThread().isInterrupted()`) both before and after the compiler stage. The server (`JavelleLanguageServerMain`) runs all analysis/request work on a single-thread `ExecutorService`, tracks each request in a `pending` map as a `FutureTask`, and `$/cancelRequest` calls `task.cancel(true)` (a real thread interrupt) then races a `terminal` `Set` against the normal completion path to guarantee exactly one terminal response. Independently re-read and re-ran (thrice) `P10BlackBoxTest.cancelledOldAnalysisCannotPublishAndNewerDiagnosticWinsOnce`, which sets `JAVELLE_LSP_TEST_ANALYSIS_CHECKPOINT=3` (the first checkpoint call belonging to the second, hover-triggered `analyze()` invocation — i.e. the worker thread has genuinely dequeued and begun the hover's analysis, not merely queued it), waits on a real marker file to prove the worker is blocked *inside* that running analysis, then sends `$/cancelRequest` for the hover concurrently with a newer `didChange`. It asserts, in the order actually observed off the wire: exactly one `-32800` response for the cancelled hover id, and exactly one `publishDiagnostics` at the strictly newer version (never the stale intermediate version, never a duplicate of either). The tooling-level unit test `cooperativeCancellationStopsAtInjectedCompilerStageCheckpoint` independently confirms the same interrupt→`CancellationException` behavior at the `DocumentWorkspace` layer alone. This is a deterministic race on a real running-analysis interrupt, not a queued-task cancellation dressed up as one — a materially different and stronger test than R2/R3 observed. |
...
| P10-10 | PASS | All server acceptance tests launch the installed `javelle-lsp` executable and exchange independently framed raw bytes; no test invokes a handler directly. |
...
- `JAVA_HOME=/usr/lib/jvm/java-25-openjdk PATH=/usr/lib/jvm/java-25-openjdk/bin:$PATH ./gradlew --no-daemon --dependency-verification=strict :language-server:clean :language-server:installDist :language-tooling:test :language-server:test --rerun-tasks` — exit `0`; `language-tooling` `TEST-org.javelle.language.tooling.P10DocumentWorkspaceTest.xml` reports `tests="5" failures="0" errors="0"`; `language-server` `TEST-org.javelle.language.server.P10BlackBoxTest.xml` reports `tests="12" failures="0" errors="0"`.
