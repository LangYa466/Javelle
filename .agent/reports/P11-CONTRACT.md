...
Section 3 of this frozen contract states the `LspServerDescriptor` "sends the same `initializationOptions.javelle.workspaceModelUri` shape P10's server already accepts, sourced from the P09 Gradle plugin's exported `build/javelle/workspace/main.json` when present (best-effort: if no exported model exists for the opened project, the server still starts with no workspace configured, matching P10's documented fallback)."
...
- Manual `unzip` extraction to `/tmp/p11-zip-test`, `test -x .../bin/javelle-lsp` → executable; ran `./javelle-lsp --stdio` with a hand-crafted framed `initialize` request piped over stdin → real `result.capabilities` JSON returned.
...
- Independently disassembled the resolved `com.intellij.platform.lsp.api.LspClientDescriptor`/`LspServerDescriptor` classes with `javap -p` to confirm `createInitializationOptions()` is a real, overridable extension point that `JavelleLspServerDescriptor` does not use.

### Gaps that must be fixed before P11 can be accepted

1. **Contract section 3 (initializationOptions wiring) is unimplemented.** `JavelleLspServerDescriptor` must override `createInitializationOptions()` to read the P09-exported `build/javelle/workspace/main.json` (when present) and forward it as `{"javelle": {"workspaceModelUri": ...}}`, matching what P10's server already parses (`JavelleLanguageServerMain` line ~207-209). Currently no such code exists anywhere in `intellij-plugin/src`.
2. **P11-04's test doesn't check what section 5 requires.** `P11FileTypeAndLspWiringTest.testLspServerSupportProviderOffersDescriptorForJavelleFiles` must additionally call the offered descriptor's `createCommandLine()` and assert the resulting command line references the bundled `javelle-lsp` launcher path (e.g. contains `javelle-lsp` under the plugin's own path, or matches `JavelleLspServerDescriptor.launcherPath()`), not just `isSupportedFile`.
...
**Final result: REJECT P11.** Blocking: contract §3 `initializationOptions.javelle.workspaceModelUri` wiring is entirely missing from the implementation despite being claimed done in this file's own "Implementation summary," and the P11-04 headless test does not assert the specific command-line-references-bundled-launcher requirement that section 5 calls for.
