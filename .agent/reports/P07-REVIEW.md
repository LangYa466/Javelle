...
|---|---|---|
| P07-01 | **FAIL** | `installDist` succeeds, but produces only `bin/compiler-cli` and `bin/compiler-cli.bat`; the required installed executable `javelle` does not exist. Version/help content is stable only when invoking the wrongly named launcher. |
...
| P07-10 | PASS | Help lists only P07 commands; format/migrate/inspect/javelle-lsp each exit 6 when directly invoked. |
...
| P07-12 | **FAIL** | Reviewer successfully ran the distribution from a non-repository directory with spaces/中文/emoji, but only via `compiler-cli`; the declared `javelle` packaged launcher is absent, so the required consumer invocation cannot be reproduced. |
...
- Non-repository packaged-process commands: version/help/doctor/check/compile/emit/explain ran from `.agent/logs/P07-REVIEW/nonrepo space 中文😀`. Check exit 0; semicolon check exit 3 with `JV-SYN-0001`; compile/emit exit 0; unknown explain exit 2.
- `emit-java --diagnostics json` stdout begins with `/home/langya/IdeaProjects/Javelle/.../User.java` and only then emits the envelope. `--timeout-ms 1` exit 0. These are direct counterexamples to P07-04/05/09.
- All 20 diagnostic catalog entries were independently passed to the installed process; 20/20 exited 0.
- Fresh product tree contains `compiler-cli`/`.bat`, five jars, and no `javelle` launcher.
...
|---|---|---|
| P07-01 | PASS | Clean distribution contains only the correctly named POSIX `javelle` and Windows `javelle.bat` launchers; version and root/subcommand help are stable. Windows artifact/classpath/`%*` quoting is inspected at P07; native Windows execution remains explicitly deferred to P52. |
...
|---|---|---|
| P07-01 | **FAIL** | Clean distribution now correctly contains only `bin/javelle` and `bin/javelle.bat`; POSIX version/root/subcommand help pass from a non-repository path. The required Windows launcher argument/quoting fixture is still absent, so both launchers are not independently verified. |
...
| P07-12 | PASS | Reviewer independently ran `bin/javelle` from a non-repository directory containing spaces, Traditional Chinese and emoji, with only the distribution/JDK and fixture arguments. |
...
- Fake selected JDK negative: executable `fake-jdk/bin/javac` that exits 99; `javelle doctor --jdk <fake> --format json` returned `status=SUCCESS`, exit 0.
