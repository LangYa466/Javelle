...
import dev.teyru.java.resolver.JavaPathResolver;
...
public final class DefaultCompilerDriver implements CompilerDriver {
  private static final String MANIFEST = "META-INF/teyru/generated-files-v1.json";
  ...
  public CompileResult compile(CompileRequest request, CancellationToken cancellation) {
    ...
    try {
      ...
      tracker.checkpoint();
      for (SourceInput input : request.teyruSources()) {
      ...
      staging = Files.createTempDirectory(parent, ".teyru-compile-");
      ...
      if (compiler == null) {
        messages.add(toolMessage("TY-TOOL-0001", "system Java compiler is unavailable"));
        return new CompileResult(false, -1, generated, messages, Optional.empty());
      }
      ...
      messages.add(toolMessage("TY-RESOURCE-0001", e.getMessage()));
      return new CompileResult(false, -1, generated, messages, Optional.empty());
  ...
  private static void validate(CompileRequest request) throws IOException {
    ...
    if (request.enableProcessors() || !request.processorPath().isEmpty())
      throw new IllegalArgumentException("TY-DEV-0001(annotation-processing)");
  ...
  private static JavacMessage remap(
    ...
    return new JavacMessage(
        new DiagnosticCode("TY-JAVAC-" + severity(d.getKind()).name()),
        severity(d.getKind()),
        uri,
  ...
  private static JavacMessage frontendMessage(
      dev.teyru.compiler.core.diagnostic.Diagnostic d, SourceFile source) {
    int start =
