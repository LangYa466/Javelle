...
public final class CliCompilerFacade {
  public List<SourceInput> loadProject(Path modelPath) throws java.io.IOException {
    ...
    for (var module : model.modules())
      for (var sourceSet : module.sourceSets())
        for (var rootRef : sourceSet.teyruRoots()) {
                    ...
                    .filter(
                        p ->
                            java.nio.file.Files.isRegularFile(p)
                                && p.toString().endsWith(".teyru"))
                    .sorted()
                    .toList()) {
              if (java.nio.file.Files.isSymbolicLink(source))
                throw new IllegalArgumentException("TY-WS-PATH-SYMLINK");
              result.add(
                  new SourceInput(
    ...
    if (result.isEmpty()) throw new IllegalArgumentException("TY-WS-NO-SOURCES");
  ...
  /** Compiles Teyru jointly with explicitly declared Java analysis/compile sources. */
  public Result compile(
  ...
  private static String sourceMapJson(dev.teyru.compiler.core.lowering.GeneratedFile file) {
    var out =
  ...
  private static Metadata metadata(dev.teyru.compiler.core.diagnostic.Diagnostic d) {
    String related =
        d.related().stream()
            .map(
                r ->
