...
class P06CompilerDriverTest {
  ...
  private CompileRequest request(String teyru, List<Path> java, Path generated, Path classes) {
    return new CompileRequest(
        List.of(new SourceInput("demo/User.teyru", teyru.getBytes(StandardCharsets.UTF_8))),
        java,
        List.of(),
        List.of(),
        generated,
  ...
  void frontendRawUtf16DiagnosticIsConvertedToCodePointLocation() {
    ...
    assertEquals("TY-SYN-0001", message.code().value());
    assertEquals(
  ...
  void javacFailureIsStructuredAndDoesNotPublishCandidate() throws Exception {
    ...
    assertTrue(
        result.diagnostics().stream()
            .anyMatch(m -> m.severity() == dev.teyru.compiler.core.diagnostic.Severity.ERROR));
  ...
  void staleOwnedFilesAreRemovedWhileForeignFilesSurvive() throws Exception {
    ...
    var second =
        new CompileRequest(
            List.of(new SourceInput("demo/Other.teyru", other.getBytes(StandardCharsets.UTF_8))),
            List.of(),
            List.of(),
            List.of(),
            generated,
  ...
  void release25AndRepeatedCompilationAreDeterministic() throws Exception {
    ...
    try (var children = Files.list(root)) {
      assertTrue(
          children.noneMatch(
              path -> path.getFileName().toString().startsWith(".teyru-compile-")));
    }
  }

  @Test
  void generatedJavacErrorMapsBackToTeyruOrigin() {
    ...
    JavacMessage javac =
        result.diagnostics().stream()
            .filter(message -> message.code().value().startsWith("TY-JAVAC-"))
            .findFirst()
            .orElseThrow();
    assertEquals("Bad.java", javac.generatedUri());
    assertFalse(javac.originalLocations().isEmpty(), javac.toString());
    assertEquals(
        "demo/User.teyru", javac.originalLocations().getFirst().source().workspaceRelativeUri());
    assertTrue(result.outputs().isEmpty());
  }
  ...
  void publishFaultRollsBackBothRootsAndManifest() throws Exception {
    for (String phase :
      ...
      String oldManifest =
          Files.readString(classes.resolve("META-INF/teyru/generated-files-v1.json"));
      String other = "package demo\nclass Other {\nString value\n}\n";
      CompileRequest replacement =
          new CompileRequest(
              List.of(
                  new SourceInput("demo/Other.teyru", other.getBytes(StandardCharsets.UTF_8))),
              List.of(),
              List.of(),
              List.of(),
              generated,
  ...
  void corruptManifestAndDescendantSymlinkFailClosed() throws Exception {
    Path generated = root.resolve("generated");
    Path classes = root.resolve("classes");
    Files.createDirectories(classes.resolve("META-INF/teyru"));
    Files.writeString(classes.resolve("META-INF/teyru/generated-files-v1.json"), "corrupt");
    ...
    assertEquals(
        "corrupt", Files.readString(classes.resolve("META-INF/teyru/generated-files-v1.json")));

    Files.delete(classes.resolve("META-INF/teyru/generated-files-v1.json"));
  ...
  void changedOwnedOutputAndOutputBudgetFailWithoutPublication() throws Exception {
    ...
    assertTrue(
        exceeded.diagnostics().stream()
            .anyMatch(message -> message.code().value().equals("TY-RESOURCE-0001")));
    ...
    assertEquals(
        List.of(
            new dev.teyru.compiler.core.source.TextRange(
                dev.teyru.compiler.core.source.OffsetUnit.UNICODE_CODE_POINT, 180, 198),
            new dev.teyru.compiler.core.source.TextRange(
                dev.teyru.compiler.core.source.OffsetUnit.UNICODE_CODE_POINT, 259, 278)),
        mapped.stream().map(JavacMessage::generatedRange).toList());
    assertEquals(
        List.of(
            new dev.teyru.compiler.core.source.TextRange(
                dev.teyru.compiler.core.source.OffsetUnit.UNICODE_CODE_POINT, 33, 53),
            new dev.teyru.compiler.core.source.TextRange(
                dev.teyru.compiler.core.source.OffsetUnit.UNICODE_CODE_POINT, 55, 81)),
  ...
  void missingTypeAndSyntheticClassDiagnosticsUseMappedOwners() {
    for (String source :
      ...
      JavacMessage mapped =
          result.diagnostics().stream()
              .filter(message -> message.code().value().equals("TY-JAVAC-ERROR"))
              .filter(message -> !message.originalLocations().isEmpty())
              .findFirst()
