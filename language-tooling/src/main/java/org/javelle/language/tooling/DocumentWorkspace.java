...
import org.javelle.workspace.model.WorkspaceModel;
...
public final class DocumentWorkspace implements AutoCloseable {
  ...
  static Runnable testAnalysisDelayFromEnvironment() {
    String marker = System.getenv("JAVELLE_LSP_TEST_ANALYSIS_MARKER");
    String target = System.getenv("JAVELLE_LSP_TEST_ANALYSIS_CHECKPOINT");
  ...
  public List<Message> analyze(Snapshot snapshot) {
    ...
    Path temp = null;
    try {
      temp = Files.createTempDirectory("javelle-lsp-analysis-");
      var javelleSources = new ArrayList<SourceInput>();
      javelleSources.add(
          new SourceInput(
              relative(snapshot.uri()), snapshot.text().getBytes(StandardCharsets.UTF_8)));
      for (Path source : workspacePaths(workspace, true, ".javelle"))
  ...
  private static List<Path> workspaceRoots(Optional<WorkspaceModel> workspace, boolean javelle) {
    return workspace
        .map(
            model ->
                model.modules().stream()
                    .flatMap(x -> x.sourceSets().stream())
                    .flatMap(x -> (javelle ? x.javelleRoots() : x.javaRoots()).stream())
                    .flatMap(x -> safeResolve(model, x))
  ...
  private static List<Path> workspacePaths(
      Optional<WorkspaceModel> workspace, boolean javelle, String suffix) throws IOException {
    var found = new ArrayList<Path>();
    for (Path root : workspaceRoots(workspace, javelle))
