...
final class WorkspaceJson {
  static JsonValue.ObjectValue parse(byte[] bytes, WorkspaceReadOptions limits) {
    if (bytes.length > limits.maxBytes()) throw new IllegalArgumentException("TY-WS-LIMIT-BYTES");
    String text;
    try {
      text =
      ...
      throw new IllegalArgumentException("TY-WS-SCHEMA-UTF8", e);
    }
  ...
  private static final class Parser {
    ...
    IllegalArgumentException bad(String m) {
      return new IllegalArgumentException("TY-WS-SCHEMA-JSON at " + i + ": " + m);
    }
