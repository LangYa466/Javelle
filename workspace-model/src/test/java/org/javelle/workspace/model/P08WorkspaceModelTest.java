...
class P08WorkspaceModelTest {
  ...
  private static WorkspaceModel model() {
    ...
    var m =
        new WorkspaceModel(
            ...
            List.of(
                new DocumentOverlay(
                    "file:///workspace/src/main/javelle/User.javelle",
                    1,
                    sha("class User {}"),
                    H,
                    OverlayState.DIRTY,
    ...
    var codes =
        new DefaultWorkspaceModelValidator()
            .validate(invalid, ValidationEnvironment.noIo()).stream()
                ...
                .toList();
    assertTrue(codes.contains("JV-WS-PATH-TRAVERSAL"));
    assertTrue(codes.contains("JV-WS-DUPLICATE-PATH"));
    assertTrue(codes.contains("JV-WS-TRUST-UNTRUSTED"));
    assertTrue(codes.contains("JV-WS-CYCLE-BUILD"));
    assertTrue(codes.contains("JV-WS-STALE-FINGERPRINT"));
  }
  ...
  void validatorRejectsProcessorToolchainCaseOverlapUriAndOverlayFailures() {
    ...
    assertTrue(codes.contains("JV-WS-TRUST-PROCESSOR-PATH"));
    assertTrue(codes.contains("JV-WS-SCHEMA-TOOLCHAIN-RELEASE"));
    assertTrue(codes.contains("JV-WS-PATH-CASE-AMBIGUOUS"));
    assertTrue(codes.contains("JV-WS-PATH-SOURCE-OVERLAP"));
    assertTrue(codes.contains("JV-WS-PATH-OVERLAY-URI"));
    assertTrue(codes.contains("JV-WS-OVERLAY-VERSION"));
    assertTrue(codes.contains("JV-WS-OVERLAY-CONTENT-HASH"));
    assertTrue(codes.contains("JV-WS-STALE-OVERLAY-BASE"));
  }
    ...
    var codes =
        new DefaultWorkspaceModelValidator()
            .validate(m, ValidationEnvironment.noIo()).stream()
                .map(WorkspaceDiagnostic::code)
                .toList();
    assertTrue(codes.contains("JV-WS-SCC-SOURCE-VISIBILITY"));
    assertTrue(codes.contains("JV-WS-DEPENDENCY-SOURCESET"));
    assertFalse(codes.contains("JV-WS-CYCLE-BUILD"));
  }
  ...
  void publicApiGoldenAndEveryProductionClassfileStayPlatformNeutral() throws Exception {
    ...
    try (var files = Files.walk(root.resolve("org/javelle/workspace/model"))) {
