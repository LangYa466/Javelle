...
func Emit(p *sema.Program) string {
	e := &Emitter{prog: p, strings: map[string]int{}, locals: map[*ast.Var]string{}}
	e.run()
...
func fnName(cl *ast.Class, m *ast.Method, idx int) string {
	...
	if m.Accessor != nil {
		if m.Accessor.IsSet {
			name = "set_" + m.Accessor.Prop.Name
		} else {
			name = "get_" + m.Accessor.Prop.Name
		}
	}
	return "M_" + mangle(cl.Full) + "_" + mangle(name) + "_" + fmt.Sprint(idx)
}
