...
public record Fix(String id, String title, FixKind kind, List<TextEdit> edits) {
  ...
  public void validateBounds(Map<dev.teyru.compiler.core.source.SourceId, Integer> lengths) {
    for (var edit : edits) {
      Integer length = lengths.get(edit.source());
      if (length == null
          || edit.range().unit() != dev.teyru.compiler.core.source.OffsetUnit.RAW_UTF16
