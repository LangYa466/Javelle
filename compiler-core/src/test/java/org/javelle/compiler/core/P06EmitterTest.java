...
class P06EmitterTest {
  ...
  private static BoundCompilationUnit fixture() {
    ...
    var source =
        SourceFile.decode(
            SourceId.forContent("demo/User.javelle", bytes),
            bytes,
  ...
  void parseBindEmitJavacAndJvmEndToEnd() throws Exception {
    ...
    Path root = Files.createTempDirectory("javelle-bind-e2e-");
    try {
  ...
  void binderFailsClosedForUnresolvedAndUnsupportedInput() {
    for (String text :
      ...
      var source =
          SourceFile.decode(
              SourceId.forContent("Bad.javelle", bytes),
              bytes,
              new ResourceBudget(100000, 1000, 1000, 100, 100, Long.MAX_VALUE));
      var parsed =
          new JavelleFrontend()
      ...
      assertTrue(
          binding.diagnostics().stream()
              .anyMatch(d -> d.severity() == org.javelle.compiler.core.diagnostic.Severity.ERROR));
    }
  }
  ...
  void computedPropertyHasNoStorageAndKeepsAccessorVisibility() {
    ...
    var source =
        SourceFile.decode(
            SourceId.forContent("Computed.javelle", bytes),
            bytes,
            new ResourceBudget(100000, 1000, 1000, 100, 100, Long.MAX_VALUE));
    var parsed =
        new JavelleFrontend()
  ...
  void crlfEmojiMapUsesCodePointsAndFindsExactReturnedExpression() {
    ...
    var source =
        SourceFile.decode(
            SourceId.forContent("emoji/Bad.javelle", bytes),
            bytes,
            new ResourceBudget(100000, 1000, 1000, 100, 100, Long.MAX_VALUE));
  ...
  void readableEmissionIsDeterministicMappedAndCompilesForReal() throws Exception {
    ...
    String expected =
        "// Generated by Javelle 0.1-test\n// Source: demo/User.javelle sha256="
    ...
    Path root = Files.createTempDirectory("javelle p06 測試 ");
    try {
  ...
  void unsupportedAndTypeMismatchFailClosed() {
    ...
    assertTrue(
        new EarlyTypeChecker()
            .check(changed).stream().anyMatch(d -> d.code().value().equals("JV-TYP-0001")));
    var compound =
        new Binary(
            "+=",
    ...
    assertTrue(
        new EarlyTypeChecker()
            .check(unsupported).stream().anyMatch(d -> d.code().value().equals("JV-DEV-0001")));
