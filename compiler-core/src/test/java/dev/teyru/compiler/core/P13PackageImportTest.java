...
class P13PackageImportTest {
  ...
  void packageDeclaration() {
    var result = parse("package dev.teyru.demo\nclass C {\n}\n");
    assertTrue(result.diagnostics().isEmpty(), result.diagnostics().toString());
    assertTrue(anchors(result.ast()).contains("PackageDeclaration:dev.teyru.demo"));
  }
  ...
  void multipleImportsAndPackageTogether() {
    var result =
        parse(
            "package dev.teyru.demo\nimport java.util.List\nimport java.util.*\nclass C {\n}\n");
    assertTrue(result.diagnostics().isEmpty(), result.diagnostics().toString());
    var all = anchors(result.ast());
    assertTrue(all.contains("PackageDeclaration:dev.teyru.demo"));
