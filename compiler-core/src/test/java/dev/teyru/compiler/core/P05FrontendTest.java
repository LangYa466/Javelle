...
class P05FrontendTest {
  ...
  void parsesSelectedPositiveAndNegativeCases() {
    ...
    assertEquals("TY-SYN-0001", semi.diagnostics().getFirst().code().value());
    assertEquals(
    ...
    assertTrue(nul.diagnostics().stream().anyMatch(d -> d.code().value().equals("TY-TYP-0002")));
    var field = parse("class C { private var x = 1\n}");
    assertTrue(field.diagnostics().stream().anyMatch(d -> d.code().value().equals("TY-TYP-0003")));
  }
  ...
  void unsupportedAndIncompleteFailExplicitly() {
    ...
    assertTrue(
        incomplete.diagnostics().stream().anyMatch(d -> d.code().value().equals("TY-SYN-0002")));
  ...
  void lexerConservesRawUnicodeAndProtectsDataSemicolons() {
    ...
    var source =
        SourceFile.decode(
            SourceId.forContent("lex.teyru", bytes),
            bytes,
  ...
  void ifParametersVisibilityComputedFieldAndRecoveryAreConcrete() {
    ...
    assertTrue(
        property.diagnostics().stream().anyMatch(d -> d.code().value().equals("TY-PROP-0006")));
    var rhs = parse("class C { Object m() {\n var x =\n }}");
    assertTrue(rhs.diagnostics().stream().anyMatch(d -> d.code().value().equals("TY-SYN-0002")));
  ...
  void everyFrozenUnsupportedRepresentativeIsSingleAndBalanced() {
    ...
    for (var sample : samples.entrySet()) {
      ...
      var development =
          result.diagnostics().stream()
              .filter(d -> d.code().value().equals("TY-DEV-0001"))
              .toList();
      ...
      assertTrue(
          ...
          sample.getKey());
      assertTrue(
          result.diagnostics().stream().noneMatch(d -> d.code().value().equals("TY-SYN-0002")),
  ...
  void methodParameterVarargsIsExactlyOneBalancedDevelopmentUnsupported() {
    ...
    assertEquals(
        1,
        result.diagnostics().stream().filter(d -> d.code().value().equals("TY-DEV-0001")).count());
    assertEquals(
        0,
        result.diagnostics().stream().filter(d -> d.code().value().startsWith("TY-SYN-")).count());
    var unsupported =
