...
import org.javelle.gradle.workspace.*;
import org.javelle.workspace.model.DefaultWorkspaceModelCodec;

/** Writes the accepted P08 portable workspace schema from declared, neutral inputs. */
public abstract class ExportJavelleWorkspaceModel extends DefaultTask {
  @Input
  ...
  @Input
  public abstract ListProperty<String> getJavelleRoots();
  ...
  public void write() throws Exception {
    ...
    var sourceSet =
        new SourceSet(
            ...
            refs(getJavelleRoots().get()),
            List.of(ref("build/generated/sources/javelle/" + getSourceSetName().get())),
            ...
            Optional.of(
                hash(
                    String.join("\0", getJavaRoots().get())
                        + String.join("\0", getJavelleRoots().get()))));
    var tool =
        new Toolchain(
            new PathRef(
  ...
  private static List<PathRef> refs(List<String> paths) {
    return paths.stream().sorted().map(ExportJavelleWorkspaceModel::ref).toList();
  }
