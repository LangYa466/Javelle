...
/** Incremental, isolated Javelle-to-Java generation task. */
public abstract class GenerateJavelleJava extends DefaultTask {
  @InputFiles
  @PathSensitive(PathSensitivity.RELATIVE)
  public abstract ConfigurableFileCollection getJavelleSources();
  ...
  public void generate() {
    ...
    if (!executable.isFile() || !executable.canExecute())
      throw new org.gradle.api.GradleException("Javelle compiler is not executable: " + executable);
    if (getTargetRelease().get() != 21 && getTargetRelease().get() != 25)
      throw new org.gradle.api.GradleException("Javelle targetRelease must be 21 or 25");
    getWorkerExecutor()
        .processIsolation()
        .submit(
            GenerateJavelleWork.class,
            p -> {
              p.getCompilerExecutable().set(getCompilerExecutable());
              p.getSources().from(getJavelleSources());
