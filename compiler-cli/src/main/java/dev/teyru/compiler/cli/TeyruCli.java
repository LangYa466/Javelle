...
package dev.teyru.compiler.cli;
...
public final class TeyruCli {
  private static final String VERSION = "Teyru 0.1.0 (language 1, Java 21-25)";
  private static final Set<String> LATER = Set.of("format", "migrate", "inspect", "teyru-lsp");
  private static final AtomicBoolean CANCELLED = new AtomicBoolean();

  private TeyruCli() {}

  public static void main(String[] args) {
    Runtime.getRuntime().addShutdownHook(new Thread(() -> CANCELLED.set(true), "teyru-cancel"));
    int exit;
    try {
      exit = run(args, System.out, System.err);
    } catch (IOException io) {
      err(System.err, "TY-CLI-IO: " + safe(io.getMessage()));
      exit = 4;
    } catch (RuntimeException defect) {
      err(System.err, "TY-CLI-INTERNAL: internal compiler error");
      exit = 5;
    }
    System.exit(exit);
  }
  ...
  static int run(String[] args, PrintStream out, PrintStream error) throws IOException {
    ...
    if (LATER.contains(command)) {
      err(error, "TY-CLI-NOT-AVAILABLE: " + command + " is not available in this version");
      return 6;
    }
    ...
    if ("1".equals(System.getenv("TEYRU_INTERNAL_TEST_FAULT")))
  ...
  private static int doctor(Arguments a, PrintStream out, PrintStream error) {
    ...
    else err(error, "TY-CLI-JDK: " + detail);
    return healthy ? 0 : 4;
  }
  ...
  private static int explain(Arguments a, PrintStream out, PrintStream error) {
    ...
    try {
      ...
      err(error, "TY-CLI-CATALOG: diagnostic catalog is unavailable");
      return 4;
    }
    if (text == null) return usage(error, "TY-CLI-UNKNOWN-DIAGNOSTIC: " + code);
  ...
  private static String catalogExplanation(String requested) throws IOException {
    try (var stream = TeyruCli.class.getResourceAsStream("/catalog.json")) {
  ...
  private static int compile(String command, Arguments a, PrintStream out, PrintStream error)
    ...
    if (a.sources.isEmpty() && a.project == null && !command.equals("emit-java"))
      return usage(error, "at least one .teyru file or --project is required");
    ...
    if (a.sources.stream().anyMatch("-"::equals)) {
      err(error, "TY-CLI-NOT-AVAILABLE: stdin source is unavailable");
      return 6;
    }
    ...
    if (a.project != null) {
      try {
        ...
        err(error, "TY-CLI-MODEL: " + safe(invalidModel.getMessage()));
        return 4;
      }
    }
    for (String value : a.sources) {
      Path path = Path.of(value).toAbsolutePath().normalize();
      if (!value.endsWith(".teyru") || !Files.isRegularFile(path) || Files.isSymbolicLink(path)) {
        err(error, "TY-CLI-INPUT: invalid source");
        return 4;
      }
    ...
    for (String value : a.javaSources) {
      ...
      if (!value.endsWith(".java") || !Files.isRegularFile(path) || Files.isSymbolicLink(path)) {
        err(error, "TY-CLI-INPUT: invalid Java analysis source");
        return 4;
      }
      javaInputs.add(path);
    }
    for (String value : a.classpath) {
      ...
      if (!Files.exists(path) || Files.isSymbolicLink(path)) {
        err(error, "TY-CLI-INPUT: invalid classpath entry");
        return 4;
      }
      classpath.add(path);
    }
    Path output =
        (a.output == null ? Path.of("teyru-out") : Path.of(a.output))
            .toAbsolutePath()
            .normalize();
    ...
    for (String value : a.sources)
      if (output.startsWith(Path.of(value).toAbsolutePath().normalize())) {
        err(error, "TY-CLI-OUTPUT: output aliases source");
        return 4;
      }
    Path scratch = Files.createTempDirectory("teyru-cli-");
    var cleanup = new Thread(() -> deleteTree(scratch), "teyru-cli-cleanup");
    Runtime.getRuntime().addShutdownHook(cleanup);
    try {
      if (a.timeoutMillis == 0) {
        err(error, "TY-CLI-TIMEOUT: operation timed out");
        return 124;
      }
      String hold = System.getenv("TEYRU_SIGNAL_TEST_HOLD_MILLIS");
      if (hold != null) {
        try {
      ...
      if (!result.success()) {
        ...
        if (result.diagnostics().stream().anyMatch(x -> x.code().startsWith("TY-RESOURCE-")))
          return 124;
        if (result.diagnostics().stream().anyMatch(x -> x.code().startsWith("TY-TOOL-"))) return 4;
        return 3;
      }
  ...
  private static void publishJava(
    ...
    Path candidate = parent.resolve("." + output.getFileName() + ".teyru-stage-" + nonce);
    Path backup = parent.resolve("." + output.getFileName() + ".teyru-backup-" + nonce);
    ...
    try {
      ...
      if (Files.exists(output)) {
        ...
        Path manifest = output.resolve("META-INF/teyru/generated-files-v1.json");
        ...
        Files.deleteIfExists(candidate.resolve("META-INF/teyru/generated-files-v1.json"));
      }
      ...
      publishFault("write");
      Path manifest = candidate.resolve("META-INF/teyru/generated-files-v1.json");
  ...
  private static void publishFault(String phase) throws IOException {
    if (phase.equals(System.getenv("TEYRU_PUBLISH_TEST_FAULT")))
  ...
  private static String commandHelp(String c) {
    return "Usage: teyru "
        ...
        + (c.equals("explain")
            ...
            : c.equals("doctor")
                ? " [--format human|json]"
                : " <file.teyru> [--diagnostics human|json] [--output <dir>]")
        + "\n";
  }

  private static int usage(PrintStream e, String m) {
    err(e, "TY-CLI-USAGE: " + m);
    return 2;
  }
