...
class P07BlackBoxTest {
  ...
  void packagedVersionHelpDoctorAndUnavailableAreStable() throws Exception {
    ...
    var explanation = run("explain", "TY-COMP-0002");
    ...
    assertResult(
        6, "", "TY-CLI-NOT-AVAILABLE: migrate is not available in this version\n", run("migrate"));
  }

  @Test
  void checkCompileAndEmitJavaUsePackagedDriver() throws Exception {
    Path source = root.resolve("來源 空間 😀/User.teyru");
    ...
    assertTrue(Files.isRegularFile(emitted.resolve("META-INF/teyru/generated-files-v1.json")));
  ...
  void usageInputAndCompilationFailuresHaveDistinctCodes() throws Exception {
    ...
    assertEquals(4, run("check", root.resolve("missing.teyru").toString()).exit);
    Path bad = root.resolve("Bad.teyru");
    ...
    assertTrue(failure.out.contains("TY-SYN-0001"));
  ...
  void acceptedP08ProjectModelIsConsumedWithoutBuildEvaluation() throws Exception {
    ...
    Path source = project.resolve("src/main/teyru/User.teyru");
    Files.createDirectories(source.getParent());
    Files.writeString(source, validSource());
  ...
  void timeoutInternalToolchainAndSignalUseFrozenExitCodesWithoutResidue() throws Exception {
    Path source = root.resolve("Signal.teyru");
    ...
    var internal = runEnv(Map.of("TEYRU_INTERNAL_TEST_FAULT", "1"), "check", source.toString());
    assertEquals(5, internal.exit);
    assertEquals("TY-CLI-INTERNAL: internal compiler error\n", internal.err);
    ...
    builder.environment().put("TEYRU_SIGNAL_TEST_HOLD_MILLIS", "60000");
    ...
    try (var children = Files.list(tmp)) {
      assertTrue(children.noneMatch(x -> x.getFileName().toString().startsWith("teyru-cli-")));
    }
  }

  @Test
  void failedReplacementPreservesAcceptedOutput() throws Exception {
    Path good = root.resolve("Good.teyru");
    ...
    Path bad = root.resolve("Broken.teyru");
    ...
    assertFalse(Files.exists(root.resolve(".symlink-output.teyru-stage")));
    assertEquals(4, run("emit-java", good.toString(), "--output", "/proc/teyru-unwritable").exit);
  ...
  void manifestFingerprintFsyncFaultRollbackAndWindowsScriptAreInspectable() throws Exception {
    Path source = root.resolve("Publish.teyru");
    Files.writeString(source, validSource());
    ...
    for (String phase : List.of("candidate", "write", "manifest", "fsync", "move")) {
      var failure =
          runEnv(
              Map.of("TEYRU_PUBLISH_TEST_FAULT", phase),
              "emit-java",
              source.toString(),
              "--output",
              output.toString());
      ...
      try (var siblings = Files.list(output.getParent())) {
        assertTrue(
            siblings.noneMatch(
                p ->
                    p.getFileName().toString().contains(".teyru-stage-")
                        || p.getFileName().toString().contains(".teyru-backup-")));
      }
    }
    Path bat = launcher.resolveSibling("teyru.bat");
    assertTrue(Files.isRegularFile(bat));
    String script = Files.readString(bat);
    assertTrue(script.contains("dev.teyru.compiler.cli.TeyruCli %*"));
