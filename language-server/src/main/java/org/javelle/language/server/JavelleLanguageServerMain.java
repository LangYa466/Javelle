...
import org.javelle.language.tooling.DocumentWorkspace;
import org.javelle.workspace.model.*;

public final class JavelleLanguageServerMain {
  ...
  private JavelleLanguageServerMain(OutputStream out) {
    this.out = out;
  }

  public static void main(String[] args) throws Exception {
    if (args.length != 1 || !args[0].equals("--stdio")) {
      System.err.println("usage: javelle-lsp --stdio");
      System.exit(2);
    }
    try {
      new JavelleLanguageServerMain(System.out).run(System.in);
    } catch (IOException e) {
      System.err.println("javelle-lsp transport error: " + e.getMessage());
      System.exit(1);
    }
  }
      ...
      case "initialize" -> {
        ...
        respond(
            ...
            Map.of(
                ...
                capabilities(),
                "serverInfo",
                Map.of("name", "Javelle", "version", "0.1.0-SNAPSHOT")));
      }
      case "initialized" -> {
  ...
  private static Optional<WorkspaceModel> validateWorkspace(Map<String, Object> params)
    ...
    var javelle = map(options.get("javelle"));
    Object value = javelle.get("workspaceModelUri");
