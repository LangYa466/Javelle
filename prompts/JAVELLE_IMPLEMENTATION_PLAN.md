# Javelle — 完整產品實作規範與逐階段執行清單

**文件版本：1.0 · 日期：2026-09-11（UTC+8） · 性質：需要真正執行的交付契約**

> 給 Codex／其他具備真正 subagent 能力的開發代理：你不是被要求再寫一份建議書，而是依本檔實現 Javelle。先遵守根目錄 `AGENTS.md`，再從 P00 開始，沿依賴圖完成全部必要工作。腳手架、可展示原型、MVP 和未驗證的功能矩陣都不是最終交付。
>
> 本文件所有 checkbox 初始均為未完成。階段編號用於追蹤，不代表必須依數字串行：例如 P43 遷移器需先於 P39 的完整 CLI 驗收完成。本文提到的 Javelle 命令、模組、功能、網址路由與任務名稱是**要建立的產品契約**，不代表目前已經存在。
...
- 工具自身的最小執行 JDK、Gradle daemon JDK、Javelle compiler JDK、target release 與 IDEA 自帶 JBR 必須分開建模。P01 釘選可取得的 JDK 25 工作工具鏈；Java 21 profile 在對應工具鏈／API 下測試。IDEA client 不能因 server JDK 較新就與 IDE classloader 耦合。
...
- 「全部 Lombok」不包含替代 Lombok 的舊 IDE 安裝器或 patch 它所有歷史版本 javac 的機制；Javelle 必須實現全部語言功能、對應設定、必要 utility／annotation 相容與遷移能力。這個邊界必須公開，不能拿它刪除難做的 annotation。
...
7. runtime 以最少為目標而不是不誠實的絕對零：大部分生成程式不需要 Javelle runtime；若個別功能需清楚隔離的 helper，必須公開依賴與授權。
...
- 原始碼先按規範處理 Unicode escape，lexer 保留原始位置到處理後文字的映射；有意義的 `;` token 一律診斷 `JV-SYN-0001`，包含利用 Unicode escape 產生的語法分號。
...
- 跨 jar 使用 Javelle property 需版本化的 property metadata（建議 `META-INF/javelle/` 資料資源），不是靠猜所有 `getX()`。Java consumer 不需要認識這份 metadata。
...
**實現原理：**以 Javelle 自己的 symbol／AST／lowering 生成功能，Lombok 正式發行物僅作差分測試 oracle／受控遷移輔助，不得藏在正常 compiler 執行路徑中替代原生實作。
...
差分路徑：一邊 Java+官方 Lombok 編譯／必要時 delombok；另一邊 Javelle 原生 lowering+javac；比較可觀察行為、public/protected/package API、必要 private 結構、generic signatures、annotation 與 framework 結果。不要要求整個 classfile 位元組相等，也不能把 public API 差異全正規化掉。[S12]
...
- 若使用 baseline 的 MIT annotation 定義作 compile-only 相容 artifact，要隔離與保留原授權，不能同時塞入重複的 `lombok.*` classes；不得把 annotation processor 一起啟用。一般程式無 Javelle runtime 的目標不能掩蓋這類 profile 的真實依賴。
...
javelle/
├── AGENTS.md
├── prompts/JAVELLE_IMPLEMENTATION_PLAN.md
...
├── compiler-cli/              # javelle
├── language-tooling/          # editor-neutral analysis snapshots/navigation/edits
├── language-server/           # javelle-lsp process 與 LSP handlers
├── language-protocol/         # 版本化 Javelle 擴充 DTO
...
model 至少記錄 schemaVersion、workspace root、modules、source sets、Java/Javelle roots、generated roots、source JARs、classpath、module path、toolchain executable／version、source/target release、compiler options、processor path／trust policy、test/runtime dependencies、encoding、property metadata 與來源 fingerprint。
...
```text
javelle --version
javelle doctor --format json
javelle check --project <model-or-project> --diagnostics json
javelle compile --project <model-or-project>
javelle emit-java <file-or-project> --output <directory>
javelle format <paths> --check
javelle explain <diagnostic-code>
javelle migrate <paths> --dry-run --report json
javelle migrate <paths> --apply
javelle inspect <file> --kind ast|symbols|source-map --format json
javelle-lsp --stdio
```
...
Color／notebook／inline-value 等協定 family 要盤點；對沒有顏色語法、notebook host、debug adapter 的 Javelle 可合理不提供，不能為「全 LSP」造假資料。完整指必要語言服務與清楚的全 method 矩陣，不是宣稱所有客戶端可顯示規範每個選項。
...
必須解決：Java 編輯器中 `new JavelleClass()`、getter／builder completion、Ctrl+B 回 Javelle declaration、Java ↔ Javelle references／rename、未 build 的生成成員可見、避免 generated Java 與 light projection 重複 symbols。

允許 IDE-specific PSI/light classes/projection adapter，但所有 signature／property mapping／語義判斷由共用模型提供。Java host 的既有 PSI 分析不是 Javelle 第二套語義引擎；若 host API 無法呈現某能力，寫 adapter／明確支援版本，不用「LSP 有做」冒充 IDEA 完成。

Debug 是另外的交付：選擇 IDEA Java debugger position manager／JDWP 映射，或經 ADR 決定的獨立 DAP adapter。驗證 Javelle 斷點、step、locals、exceptions、多行 lambda、getter/setter／生成區域。診斷 source map 不自動等於 JVM debug strata；不能承諾任意第三方 debugger 自動映射。
...
首頁用真正 Javelle／生成 Java 並排範例，清楚展現熟悉語法，避免大量漸層卡片、AI 宣傳詞或虛構統計。正式淺／深主題、可讀 typography、手機／桌面 navigation、鍵盤操作、copy code、anchors、404、搜尋、版本選擇、英文與繁體中文核心頁。
...
sitemap、robots、canonical、meta、Open Graph、RSS／Atom 與 changelog.json 自動生成。正式域名從配置取得，未註冊不得自稱擁有 `javelle.dev`／其他域名，測試使用 localhost／example.invalid。預覽站不冒充 production；不偷偷開放上傳 Java 並執行的線上 playground。
...
OpenJDK 授權模式適用於 Javelle 自有程式碼；第三方及輸出 helper 逐項審查，不能替他人程式碼重新授權。[S20][S21] 本地 `releaseCheck` 能完成不等於已發布；外部平台憑證／signing keys／domain 是獨立授權 gate，既不偽造也不阻礙其餘本地工作。
...
- [ ] **P09-02** 註冊 src/main/javelle 與 src/test/javelle，接上 generateJavelleJava／generateTestJavelleJava。
...
- [ ] **P09-06** 提供 main/test workspace model 匯出任務，輸出在 build/javelle，不修改 src。
- [ ] **P09-07** 建立 includeBuild／本地 plugin resolution 的 consumer sample，不假定公共 Plugin Portal 已有 Javelle。
- [ ] **P09-08** 用 Gradle TestKit 跑 build/test，Java 呼叫 Javelle class，Javelle 也呼叫同 module Java class。[S08]
- [ ] **P09-09** 初步宣告 inputs/outputs，連續兩次 build 確認不是每次都全量重跑；失效細節留 P37 完成。
- [ ] **P09-10** 生成語法／型別錯誤時 Gradle build 必須失敗並顯示 .javelle 源位置。
- [ ] **P09-11** 檢查刪除 .javelle 後不留下舊 Java/class 被當成成功；cleanup 僅處理本工具 manifest。
- [ ] **P09-12** 由 reviewer 在全新 consumer 目錄按 docs build，不依賴本庫測試直接調用 internal compiler。

**階段出口：** 使用者可以透過 Gradle 寫真 .javelle 並編譯混合專案；這是第一次產品接入，不是完整 cache 驗收。 所有項目另須通過根 `AGENTS.md` 的共同 DoD。

## P10 — 獨立 LSP 啟動與最小協定閉環

**負責：** lsp  
**依賴：** P05、P07、P08  
**產物：** javelle-lsp process、lifecycle、基本診斷／hover
...
- [ ] **P10-05** 將 Javelle syntax errors 發回原始 URI/range，修正後清除舊錯誤。
...
- [ ] **P11-01** 建立 .javelle file type、language ID、圖示與基本共用 lexer／text syntax highlighting，不新增語義引擎。
- [ ] **P11-02** 依 P01 選定 IDE/LSP API 實現 client descriptor/provider，不混用不同 SDK 世代類名。[S06]
- [ ] **P11-03** 從設定或受控 bundled distribution 啟動 javelle-lsp；檢查 server version/JDK，不能默默啟動另一個未知執行檔。
- [ ] **P11-04** 打開 P09 的真 Gradle consumer，編輯 User.javelle，確認 server 收到未保存內容。
...
**階段出口：** 詞法覆蓋 Java基線與Javelle新增語法，對不完整編輯與惡意輸入有可重現防護。 所有項目另須通過根 `AGENTS.md` 的共同 DoD。
...
- [ ] **P16-07** 解析 Javelle 生成成員／property descriptors，讓編譯前的 getter/builder symbols 可被引用。
...
- [ ] **P17-01** 實作先收集 Javelle declarations／generated member shapes，再與 Java sources 聯合 attribution 的管線。
- [ ] **P17-02** Java A 引用 Javelle B、B 又引用 A 的同 module case 必須一次正常建置，不以先各自編譯造成假循環。
- [ ] **P17-03** 需要 getter／builder/member型別的Java與Javelle code都能看到一致projection，不發布stub class作產品。
...
- [ ] **P17-11** 測試 javac warnings/errors的多origin mapping、Java檔直接錯誤保持Java位置，不能強行映射成Javelle。
- [ ] **P17-12** reviewer從乾淨consumer重現循環參照與processor cases，確認不是預先殘留class救過建置。

**階段出口：** Java↔Javelle及外部processor在真實清潔構建中工作，無無界round或stub偽產物。 所有項目另須通過根 `AGENTS.md` 的共同 DoD。
...
- [ ] **P19-07** 驗證跨JAR Javelle consumer 可使用property，純Java consumer 只需普通getter/setter ABI。
- [ ] **P19-08** 生成source JAR與原始Javelle來源分開且可追溯；generated Java不加入使用者源碼目錄。
...
**階段出口：** 源碼、生成Java、class、metadata有穩定可追蹤關係，並能跨jar保留Javelle property語義。 所有項目另須通過根 `AGENTS.md` 的共同 DoD。
...
- [ ] **P20-07** 建立Java+Lombok、delombok（適用時）、Javelle原生生成三條測試路徑，oracle不能進入production compiler。[S12]
...
- [ ] **P20-11** 獨立reviewer檢驗oracle確實執行官方Lombok且Javelle正常路徑沒有偷偷調用它。
...
- [ ] **P22-11** 逐項運行官方oracle與Javelle差分，錯誤位置/診斷內容與上游約束一致到契約要求的層次。
...
- [ ] **P23-11** 驗證Java和Javelle consumer均可使用builder API，LSP能看到構造前尚未生成文件的builder符號。
...
- [ ] **P24-08** 覆蓋父子Java↔Javelle混合繼承、library jar父類與跨module consumer。
...
- [ ] **P28-11** 驗證Java/Javelle兩端的annotation navigation/hover/source-map，錯誤映射到原始onX位置。
...
- [ ] **P29-09** 驗證普通Javelle compile不啓動Lombok processor，oracle jar只在測試/明確遷移輔助路徑出現。
...
- [ ] **P30-08** 跨Java/Javelle/current buffer/外部source JAR導航使用統一URI和位置模型。
...
- [ ] **P31-07** 實現definition/declaration/typeDefinition/implementation，來源優先原始Javelle/Java/source JAR而非generated Java。
...
- [ ] **P31-10** 測試Java↔Javelle/跨module/跨JAR/未保存buffer以及private/protected不可見成員過濾。
...
**階段出口：** 補全、提示、簽名與導航能處理真實Java泛型和生成API，不只支持Javelle自己的簡單名字。 所有項目另須通過根 `AGENTS.md` 的共同 DoD。
...
- [ ] **P32-02** 診斷帶stable code、related information、doc link與準確Javelle位置；修復後清除stale errors。
...
- [ ] **P33-03** property rename同時處理Javelle訪問與Java生成getter/setter調用的對應關係，按實際命名規則生成編輯。
- [ ] **P33-04** getter/setter顯式命名與fluent/Accessors cases有清晰策略，不把用戶手寫獨立方法錯誤捆綁。
- [ ] **P33-05** 跨Java/Javelle/multi-module/unsaved buffers搜索完整引用，jar-only外部使用不能偽造源碼編輯。
...
- [ ] **P36-01** 提供獨立javelle-lsp發行目錄與啓動文檔，不要求安裝Javelle IDEA plugin。
...
- [ ] **P37-03** 聲明所有影響輸出的inputs：Java/Javelle source、Lombok config、classpath、module path、JDK/target、compiler version、options。
...
- [ ] **P38-02** 建立多module Java→Javelle→Java調用和跨jar property metadata消費測試。
...
- [ ] **P38-08** 混合項目中原Java使用Lombok的情況明確支持策略：用戶可保留Java自身processor，Javelle仍走原生路徑，防止double-generation。
- [ ] **P38-09** 測試maven-publish生成POM/module metadata、consumer runtime/compileOnly依賴以及optional helper模式。
- [ ] **P38-10** 測試Java doc generation/source attachment不會暴露臨時路徑或丟失Javelle原始source mapping。
...
- [ ] **P41-02** Java編輯器在未build時可補全Javelle classes、constructors、getters/setters、Builder/SuperBuilder成員。
- [ ] **P41-03** Java getName()/builder調用的Ctrl+B回Javelle field/property/annotation，不能只跳generated Java。
- [ ] **P41-04** Javelle編輯器導航到Java/source JAR也正確，Java/Javelle交叉reference結果一致。
- [ ] **P41-05** 處理generated Java與light elements的索引去重、緩存失效、declaring class identity與overload。
- [ ] **P41-06** IDE rename可觸發共用server重命名並應用Java/Javelle edits，collision/preview/version檢查保留。
- [ ] **P41-07** Java源未保存更改能影響Javelle completion，Javelle未保存成員更改能更新Java投影。
...
**階段出口：** Java用戶在IDEA中能自然消費Javelle與生成API，導航和重構回到原始源碼。 所有項目另須通過根 `AGENTS.md` 的共同 DoD。

## P42 — Javelle 除錯、斷點與執行來源映射
...
- [ ] **P42-03** 在Javelle原始文件設置斷點，映射到正確generated Java/class位置並能實際命中。
...
- [ ] **P42-08** console/stacktrace提供Javelle導航同時保留真實raw frame信息，不承諾任意第三方debugger自動支持。
- [ ] **P42-09** 跨Java/Javelle調用可來回step，外部JAR仍正常使用Java調試機制。
...
**階段出口：** 原始Javelle斷點與單步真實可用；source-map、LSP和debug邊界沒有混淆。 所有項目另須通過根 `AGENTS.md` 的共同 DoD。
...
- [ ] **P43-09** 遷移後使用原有Java tests和Javelle consumer真實build，驗證行為/ABI而不只比較轉換文本。
...
- [ ] **P44-02** 建立Java↔Javelle mixed-module、多module/library property metadata、Lombok Builder/SuperBuilder組合樣例。
...
- [ ] **P44-07** Java生成示例由編譯器生成並比對，顯示正常Java分號，不把Java例子也誤格式化成Javelle。
...
- [ ] **P46-02** 實現品牌首頁：Javelle名稱、主張、真實Javelle/Java對照、明確安裝入口與當前完成狀態。
...
- [ ] **P49-04** 進行parse-format-parse、Java→Javelle→Java語義測試，確認轉換不會損壞字面量/注釋。
...
- [ ] **P53-03** 驗證普通property/Data/Builder程序無不必要Javelle runtime，必要helper/profile依賴準確傳遞。
...
- [ ] **UAT-02** `src/main/javelle/Main.javelle` 編譯並執行；生成 Java 可直接閱讀，class 是正常 JVM 產物。
...
- [ ] **UAT-13** 同一 module Java 呼叫 Javelle、Javelle 呼叫 Java，互相參照的類型在 clean build 成功。
- [ ] **UAT-14** Javelle library 打成 JAR 後，另一個 Javelle consumer 透過 metadata 使用 property，Java consumer 正常使用 accessor。
...
- [ ] **UAT-17** `@Data`、`@Value`、Builder／Default／Singular／SuperBuilder 的真實 Java/Javelle consumer 與組合都可用。
...
- [ ] **UAT-21** `javelle check` 的 exit code 與 JSON diagnostics 可由程式讀取；`explain`、`emit-java`、`inspect`、`format --check` 可用。
- [ ] **UAT-22** `javelle-lsp --stdio` 在無 IDEA 安裝環境可啟動，stdout 無雜訊，取消／重啟／未保存 buffer 正確。
...
- [ ] **UAT-25** 在 Java 編輯器輸入尚未 build 的 Javelle class／builder，IDEA 能補全；Ctrl+B 回原始 `.javelle`。
- [ ] **UAT-26** Java ↔ Javelle references／rename 準確，重命名後 build/test 通過，無重複 symbols 或誤改同名字串。
- [ ] **UAT-27** 在原始 Javelle 設斷點，普通方法、property 與多行 lambda 能命中並單步、看 locals，跨 Java/Javelle 正常。
...
```text
你是 Javelle 的 <role> subagent，工作包 <Pxx-Wyy>。
...
```text
你是 Javelle 獨立 reviewer，不是本包原實作者。
...
```json
{
  ...
  "baseRevision": null,
  "ownedPaths": ["compiler-core/src/main/java/org/javelle/lowering/properties/"],
...
自訂 `.codex/agents/javelle-reviewer.toml` 的最小候選範本：

```toml
name = "javelle-reviewer"
description = "Review Javelle work packages against the specification and real test evidence."
...
```text
Project: Javelle
Normative files: AGENTS.md + prompts/JAVELLE_IMPLEMENTATION_PLAN.md
...
下列為本規範建立時核對的官方／一手資料。本文大部分內容是 **Javelle 本身的設計與驗收要求**，不是聲稱上游工具已替我們實現。版本、API、功能狀態與授權依賴在 P01 再次核對並釘選。對正式法律判斷不確定之處，保留發布阻塞與審查需求，不捏造律師結論。
...
| S03 | Oracle — [Java SE 25 Language Specification](https://docs.oracle.com/javase/specs/jls/se25/html/index.html) | Java 語言基線；Javelle 差異以本規範明列 |
...
| S19 | Astro/Starlight — [Getting Started](https://starlight.astro.build/getting-started/) | 可實作的文件站基礎，不等於現成 Javelle 官網 |
